...
import img4 from "@assets/generated_images/modern_poultry_farm_exterior.png";
...
const images = [
  ...
  { src: img4, caption: "Modern Facilities" },
            ...
            {images.map((img, idx) => (
              <div key={idx} className="group relative aspect-square overflow-hidden rounded-xl cursor-pointer">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <span className="text-white font-serif text-xl font-medium">{img.caption}</span>
                </div>
              </div>
            ))}
          </div>
